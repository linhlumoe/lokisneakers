...
class BaoKim_payment{
	...
	var $code 		= 'baokim';
	var $setting 	= array(
		'business' 				=> 'khanhlinh970@gmail.com',//tai khoan cua nguoi nhan tien
		'merchant_id' 			=> '28786',//id cua website khi dang ky ben baokim
		'secure_pass' 			=> '00cf08d1fe6dd28c',//mat khau giao tiep
		'cost_constant'			=> 1700,
		'cost_percent'			=> 1,
	);
	...
	/**
	 * Chuyen den payment
	 * @$tran_id : id cua bang order, cho biết thanh toán cho đơn hàng nào trên website
	 * @amount: số tiền khách cần thanh toán, là cột cost trong bảng order
	...
	function payment($tran_id, $amount, $return_url = '')
	{
		...
		$url = $this->_bk_create_url($tran_id, $this->setting['business'], $amount, 0, 0, $tran_info, $url['success'], $url['cancel'], $url['detail']);
		redirect($url);
	}
	...
	{
		// Luu du lieu tra ve,luu vào cột payment_info trong bảng order
		$result = $this->CI->input->post();

		$this->CI->load->model('order_model');
		$data = array();
		$data['payment_info'] = serialize($result);
		$this->CI->order_model->update($tran_id, $data);
		...
		// Kiem tra amount
		$amount_pay = intval($this->CI->input->post('total_amount'));
